...
Guide for third-party developer integration with Better Bats and DasikLibrary APIs on Minecraft 26.1.2.

---

## 📦 Gradle Dependency Setup

Add DasikLibrary to your third-party mod's `build.gradle` and `fabric.mod.json`:

```groovy
dependencies {
    modImplementation "net.dasik.social:dasik-library:1.8.38"
}
```
...
public class BatInspector {
    public static void inspectBat(Bat bat) {
        ...
        // Read movement speed trait modifier (Default baseline: 0.0f)
        ...
        System.out.println("Bat scale: " + scale + ", Speed bonus: " + speedBonus + ", Damage multiplier: " + attackDamage);
    }
}
```
...
public class BatRuleChecker {
    public static void checkRules(Level level) {
        ...
        int spawnWeight = DynamicGameRuleManager.getInt(level, BetterBatsFabric.BAT_SPAWN_WEIGHT);
    }
}
```
