...
|---|---|---|---|---|---|
| `better-bats:bat_swarm_size` | Integer | `5` | Unbounded | Bat Swarm Size | Maximum size of bat flocks. When met, bats stop recruiting new members into their murmuration. Set to `0` to disable flocking. |
...
| `better-bats:debug_mode` | Boolean | `false` | `true`/`false` | Debug Mode | Enable detailed diagnostic logging for Better Bats AI and behaviors. Resets to `false` on world/server restart. |

---

## 💻 In-Game Command Execution Examples

Server operators and singleplayer players can modify any GameRule live in-game using either the dedicated `/betterbats` command suite or standard vanilla Brigadier `/gamerule` commands:

### Using `/betterbats` Suite (Recommended)
```mcfunction
# Check overview of all active GameRules
/betterbats status

# Query specific rule value
/betterbats get bat_swarm_size

# Update rule and auto-save baseline config
/betterbats set bat_swarm_size 10

# Accelerate guano production to every 2 minutes (2400 ticks)
/betterbats set bat_guano_threshold 2400

# Enable physical bone meal drops
/betterbats set bat_drop_guano_item true

# Toggle diagnostic logging
/betterbats debug on

# Reset all GameRules to factory defaults
/betterbats reset
```

### Using Vanilla `/gamerule`
```mcfunction
# Set bat flock size limit to 10
/gamerule better-bats:bat_swarm_size 10

# Set spawn weight to 50
/gamerule better-bats:bat_spawn_weight 50
...
> ⚠️ **GameRule vs Config Sync Note**: Values modified in `.minecraft/config/better-bats.json` set baseline defaults for **NEW WORLDS ONLY**. Existing active worlds read and store their values directly inside the world's `level.dat` GameRule container. To reconfigure an existing world, use `/betterbats set <rule> <val>` (which automatically updates both the active world GameRule and the config file) or the in-game YetAnotherConfigLib (YACL) menu.
